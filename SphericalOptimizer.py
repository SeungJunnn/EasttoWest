...
class SphericalOptimizer(Optimizer):
    def __init__(self, optimizer, params, **kwargs):
        ...
        with torch.no_grad():
            ...
            self.radii = {param: (param.pow(2).sum(tuple(range(2,param.dim())),keepdim=True)+1e-9).sqrt() for param in params}
    ...
    def step(self, closure=None):
        ...
        for param in self.params:
            ...
            param.data.div_((param.pow(2).sum(tuple(range(2,param.dim())),keepdim=True)+1e-9).sqrt())
